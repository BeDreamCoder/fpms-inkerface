...
var getRegisteredUsers = function (username, userOrg) {
    var member;
    var client = getClientForOrg(userOrg);
...
for (let key in ORGS) {
    if (key.indexOf('org') === 0) {
        ...
        // cross chain need two org
        // if (key === hfc.getConfigSetting('orgName')) {
        let caUrl = ORGS[key].ca;
        caClients[key] = new copService(caUrl, null /*defautl TLS opts*/, '' /* default CA */, cryptoSuite);

        getRegisteredUsers(hfc.getConfigSetting('userName'), key).then((user) => {
...
function setupPeers(channel, org, client) {
    for (let key in ORGS[org].peers) {
        ...
        eh.setPeerAddr(ORGS[org].peers[key].events, grpcOpts);
        eh.connect();
...
function newRemotes(names, forPeers, userOrg) {
    ...
    for (let idx in names) {
        ...
        if (ORGS[userOrg].peers[peerName]) {
            ...
            if (forPeers) {
                let channel = channels[userOrg];
                channel.getPeers().forEach((peer) => {
                    if (peerName === peer.getName()) {
                        targets.push(peer);
                    }
                });
            } else {
                let channel = channels[userOrg];
                channel.getEventHubs().forEach((eh) => {
                    if (ORGS[userOrg].peers[peerName].events.includes(eh.getPeerAddr())) {
                        targets.push(eh);
                    }
                });
            }
...
function disconnectEventHub(userOrg) {
    let channel = channels[userOrg];
    channel.getEventHubs().forEach((eh) => {
        if (eh.isconnected()) {
            eh.disconnect();
...
exports.disconnectEventHub = disconnectEventHub;
