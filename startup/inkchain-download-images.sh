...
InkchainDockerPull() {
  ...
  for IMAGES in peer orderer tools ccenv javaenv couchdb kafka zookeeper; do
      ...
      echo
      docker pull evanszhang/inkchain-$IMAGES:$INKCHAIN_TAG
      docker tag evanszhang/inkchain-$IMAGES:$INKCHAIN_TAG inklabsfoundation/inkchain-$IMAGES
