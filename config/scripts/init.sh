...
installChaincode () {
    ...
    peer chaincode install -n network -v 2.2 -p ${NETWORK_CC_PATH} -o orderer.example.com:7050 >&log.txt
...
upgradeChaincode () {
    local starttime=$(date +%s)
    peer chaincode upgrade -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C ${CHANNEL_NAME} -n network -v 2.2 -c '{"Args":["init"]}' -P "OR ('Org1MSP.member')" >&log.txt
    res=$?
    cat log.txt
    verifyResult $res "Chaincode upgrade on pee0.org1 on channel '$CHANNEL_NAME' failed"
    echo_g "=========== Chaincode upgrade on peer0.org1 on channel '$CHANNEL_NAME' is successful ========== "
    echo_b "upgrade spent $(($(date +%s)-starttime)) secs"
    echo
}
...
echo_b "=====================3.Install chaincode token on Peer0/Org0========================"
installChaincode

#echo_b "=====================4.Instantiate chaincode token, this will take a while, pls waiting...==="
#instantiateChaincode

echo_b "=====================5.Upgrade chaincode token ========================="
upgradeChaincode
